...
test('HTML CSS and JS initial source bundle fits 30 KiB gzip budget',async()=>{const paths=['index.html','styles.css','store.js','catalog.js','gallery.js','motion.js'];let bytes=0;for(const p of paths)bytes+=gzipSync(await readFile(new URL(p,root))).length;assert.ok(bytes<30*1024,`${bytes} bytes`);});
