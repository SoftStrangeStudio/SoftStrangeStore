...
function renderCatalog(){
 ...
 for(const p of products){const article=el('article','product-card');const a=el('a');a.href=`product.html?slug=${encodeURIComponent(p.slug)}`;const visual=el('div','card-image');visual.append(productImage(p),el('span','badge',STATES[p.status]));const info=el('div','card-details');info.append(el('p','card-category',p.categoryLabel||'LITTLE THINGS'));const heading=el('div','card-heading');heading.append(el('h3','',p.name));const arrow=el('span','','↗');arrow.setAttribute('aria-hidden','true');heading.append(arrow);info.append(heading,el('p','',p.approved?priceLabel(p):'Sample concept · not for sale'));a.append(visual,info);article.append(a);catalog.append(article);}
